...
app.use(flash());
mongoose.connect("mongodb+srv://admin-darsala:tavgasa2003@cluster0.md00x.mongodb.net/todolistDB", {
...
itemsSchema.plugin(encrypt,{secret: process.env.SECRET, encryptedFields:["name"], requireAuthenticationCode: false});
listSchema.plugin(encrypt,{secret: process.env.SECRET, encryptedFields:["name"], requireAuthenticationCode: false});
  		...
  		if(err){
  			...
  			Item.find({}, function(e, foundItems) {
  				if(e){
  					...
  					User.findOrCreate({ username: profile.id}, {provider: "google",list: foundLists, item: foundItems, darkMode: false}, function (err, user) {
				      return cb(err, user);
				    });
  				}
  			});
  		...
  		if(err){
  			...
  			Item.find({}, function(e, foundItems) {
  				if(e){
  					...
  					User.findOrCreate({ username: profile.id}, {provider: "facebook",list: foundLists, item: foundItems, darkMode: false}, function (err, user) {
				      return cb(err, user);
				    });
  				}
  			});
		...
		User.findOne({_id: req.session.passport.user}, function(err, foundUser) {
			...
			// foundUser.item.forEach(function(single) {})
				...
				if(userList.length === 0){
					res.render("list", { listTitle: "Today", newItem: userItem, toDoList: "Today", darkMode: darkMode});
				} else{
					res.render("list", { listTitle: "Today", newItem: userItem, toDoList: userList, darkMode: darkMode});
				}
		});
	} else{
		res.redirect("/login");
		...
		if(err){
			...
			Item.find({}, function(err, foundItems) {
				if(err){
					...
					User.register({username:req.body.username, list: foundLists, item: foundItems, darkMode: false, provider:"local"}, req.body.password, function(err, user) {
						if(err){
							console.log(err);
...
app.get("/:customListName", function(req, res){
	if (req.isAuthenticated()){
		...
		if(customListName !== "Settings" && customListName !== "Redirect" && customListName !== "Favicon.ico" && customListName !== "login" && customListName !== "register" && customListName !==  'deleteList'){
			User.findOne({_id: req.session.passport.user}, function(err, foundUser){
				var userItem = foundUser.item;
				var userList = foundUser.list;
				var darkMode = foundUser.darkMode;
				if(!err){
					...
					userList.forEach(function(singleList) {
						if (singleList.name === customListName){
							a=true;
							res.render("list", { listTitle: singleList.name, newItem: singleList.items, toDoList: userList, darkMode: darkMode});
						}
					});
					if(a===false){
						const list = {
...
app.post("/", function (req, res) {
	...
	if (itemName !== "" && itemName !== defaultItems && !toDoItems.includes(itemName)){
		User.findOne({_id: req.session.passport.user}, function(err, foundUser){
			var userItem = foundUser.item;
			var userList = foundUser.list;
			const item = {
...
app.post("/Redirect", function(req, res) {
	if (req.body.checkedCondition==="true"){
		User.findOneAndUpdate({_id: req.session.passport.user}, {darkMode: true}, function(err, obj) {
			if(err){
				console.log(err);
			}
		});
		...
		User.findOneAndUpdate({_id: req.session.passport.user}, {darkMode: false}, function(err, obj) {
			if(err){
				console.log(err);
			}
		});
